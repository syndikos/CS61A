...
def run_experiments():
    ...
    if False:  # Change to False when done finding max_scoring_num_rolls
        six_sided_max = max_scoring_num_rolls(six_sided, 1000)
...
def free_bacon_timus(opponent_score):
    """Return free_bacon result after applied hogtimus"""
...
def get_score_list(dice=six_sided, num_samples=1000):
    """Return the list of scores produced by calling roll_dice from 1 time to 10 times 
    with averaged dice with num_saples times.
    """
    scores = []
    average_dice = make_averaged(roll_dice, num_samples)
    for number_of_dice in range(1, 11):
...
# def get_possibility(the_list):
#     """Return a list contains how many times a element appears in the_list."""
#     possibility = []
#     for element in the_list:
#         num = the_list.count(element)
#         possibility.append(num)
#     return possibility

# def get_most_possible(the_list):
#     """Return the element which appears the most times in the_list"""
#     possibility = get_possibility(the_list)
#     max_possibility = max(possibility)
#     index = possibility.index(max_possibility)
#     ans = the_list[index]
#     return ans

# def get_the_largest(the_list):
#     """Returns the largest element in the_list"""
#     ans = max(the_list)
#     return ans

# def get_margin(dice=six_sided, num_samples=1000):
#     """Returns the most suitable margin for the situation."""
#     the_list = get_score_list(dice, num_samples)
#     ans = get_most_possible(the_list)
#     return ans
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score < opponent_score:
        # def margin_num_rolls(margin, num_rolls):
        if select_dice(score, opponent_score) == four_sided:
            strategy = swap_strategy(score, opponent_score, 13, 4)
        elif select_dice(score, opponent_score) == six_sided:
            strategy = swap_strategy(score, opponent_score, 21, 6)

    else:
        if select_dice(score, opponent_score) == four_sided:
            strategy = swap_strategy(score, opponent_score, 8, 4)
        elif select_dice(score, opponent_score) == six_sided:
            strategy = swap_strategy(score, opponent_score, 11, 6)
    return strategy
